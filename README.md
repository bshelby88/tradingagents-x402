...
- **Price:** `$0.05` USDC per request by default, overridable by `X402_PRICE` as a positive integer in atomic micro-USDC units (`50000` = `$0.05`; 1 USDC = 1,000,000 units). `fly.toml` does not set `X402_PRICE`, and the live production challenge serves the `$0.05` (`50000`) default (verified 2026-09-17 via `POST /api/analyze-ticker` HTTP 402) — always verify the live HTTP 402 challenge before signing. Payment settles before downstream completion. Invalid, fractional, or non-finite values stop startup.
